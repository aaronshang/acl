#ifndef	__ACL_IOCTL_INCLUDE_H__
#define	__ACL_IOCTL_INCLUDE_H__
...
typedef void (*ACL_IOCTL_WORKER_FN)(ACL_IOCTL *h_ioctl, void *arg);
 ...
 * 加快事件循环过程
 * @param h_ioctl {ACL_IOCTL*} 服务器任务池句柄
 */
ACL_API void acl_ioctl_add_dog(ACL_IOCTL *h_ioctl);
 ...
 * @param ... 变参参数序列
 */
ACL_API void acl_ioctl_ctl(ACL_IOCTL *h_ioctl, int name, ...);
 ...
 * @param h_ioctl {ACL_IOCTL*} 服务器任务池句柄
 */
ACL_API void acl_ioctl_free(ACL_IOCTL *h_ioctl);
 ...
 */
ACL_API int acl_ioctl_start(ACL_IOCTL *h_ioctl);

/**
 * 消息循环(仅适用于单线程模式)
 * @param h_ioctl {ACL_IOCTL*} 服务器任务池句柄
 */
ACL_API void acl_ioctl_loop(ACL_IOCTL *h_ioctl);
 ...
 * @return {ACL_EVENT*}
 */
ACL_API ACL_EVENT *acl_ioctl_event(ACL_IOCTL *h_ioctl);

/**
 * 将数据流从事件的读、写监听中去除
 * @param h_ioctl {ACL_IOCTL*} 服务器任务池句柄
 * @param h_stream {ACL_VSTREAM*} 客户端数据流指针
 */
ACL_API void acl_ioctl_disable_readwrite(ACL_IOCTL *h_ioctl, ACL_VSTREAM *h_stream);
 ...
 * @param h_stream {ACL_VSTREAM*} 客户端数据流指针
 */
ACL_API void acl_ioctl_disable_read(ACL_IOCTL *h_ioctl, ACL_VSTREAM *h_stream);
 ...
 * @param h_stream {ACL_VSTREAM*} 客户端数据流指针
 */
ACL_API void acl_ioctl_disable_write(ACL_IOCTL *h_ioctl, ACL_VSTREAM *h_stream);
 ...
 */
ACL_API int acl_ioctl_isset(ACL_IOCTL *h_ioctl, ACL_VSTREAM *h_stream);
 ...
 * @return {int} 1：表示是; 0: 表示否
 */
ACL_API int acl_ioctl_isrset(ACL_IOCTL *h_ioctl, ACL_VSTREAM *h_stream);
 ...
 * @return {int} 1：表示是; 0: 表示否
 */
ACL_API int acl_ioctl_iswset(ACL_IOCTL *h_ioctl, ACL_VSTREAM *h_stream);
 ...
 */
ACL_API int acl_ioctl_iocp_close(ACL_IOCTL *h_ioctl, ACL_VSTREAM *h_stream);
 ...
 */
ACL_API void acl_ioctl_enable_read(ACL_IOCTL *h_ioctl,
				ACL_VSTREAM *h_stream,
				int timeout,
 ...
 *  用户需要在 callback 内将该参数转换成自己的可识别类型
 */
ACL_API void acl_ioctl_enable_write(ACL_IOCTL *h_ioctl,
				ACL_VSTREAM *h_stream,
				int timeout,
				ACL_IOCTL_NOTIFY_FN callback,
				void *context);
 ...
 * @param h_ioctl {ACL_IOCTL*} 服务器任务池句柄
 * @param h_stream {ACL_VSTREAM*} 处于连接远程服务器状态的本地客户端数据流
 * @param timeout {int} 连接超时时间
 * @param callback {ACL_IOCTL_NOTIFY_FN} 当数据流可写或出错或超时时的回调函数
 * @param context {void*} 回调函数 callback 的参数之一, 主要用于传递用户自己的参数,
 *  用户需要在 callback 内将该参数转换成自己的可识别类型
 */
ACL_API void acl_ioctl_enable_connect(ACL_IOCTL *h_ioctl,
				ACL_VSTREAM *h_stream,
				int timeout,
				ACL_IOCTL_NOTIFY_FN callback,
				void *context);
 ...
 */
ACL_API void acl_ioctl_enable_listen(ACL_IOCTL *h_ioctl,
				ACL_VSTREAM *h_stream,
				int timeout,
				ACL_IOCTL_NOTIFY_FN callback,
				void *context);
 ...
 *     获得一个客户端连接
 */
ACL_API ACL_VSTREAM *acl_ioctl_listen_ex(const char *addr, int qlen, int block_mode,
 ...
 */
ACL_API acl_int64 acl_ioctl_request_timer(ACL_IOCTL *h_ioctl,
 ...
 * @return {acl_int64} 剩余的时间, 单位为微秒.
 */
ACL_API acl_int64 acl_ioctl_cancel_timer(ACL_IOCTL *h_ioctl,                                                            
				ACL_IOCTL_TIMER_FN timer_fn,                                               
				void *context);
 ...
 */
ACL_API int acl_ioctl_add(ACL_IOCTL *h_ioctl, ACL_IOCTL_WORKER_FN callback, void *arg);
 ...
 */
ACL_API int acl_ioctl_nworker(ACL_IOCTL *h_ioctl);
