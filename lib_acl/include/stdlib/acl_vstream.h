 ...
 * @author	zsx
 * @date	2008-12-15
 * @brief	本文件中定义了关于 ACL_VSTREAM　通信流操作的类型说明及函数接口.
 * @version	2.0
 */
...
#define	ACL_VSTREAM_TYPE_LISTEN_UNIX    (1 << 4)
	...
	acl_off_t total_read_cnt;       /**< total read count of the stream */
	acl_off_t total_write_cnt;      /**< total write count of the stream */
 ...
 * @param stream {ACL_VSTREAM*} 流指针, 不能为空
 * @return ret {int}, ret > 0 OK; ret <= 0 Error
 * 注: 仅适应于网络套接字
 */
ACL_API int acl_vstream_peekfd(ACL_VSTREAM *stream);
 ...
 * @return ret {int}, ret >= 0 OK; ret < 0 Error
 */
ACL_API int acl_vstream_set_fdtype(ACL_VSTREAM *stream, int type);
...
/**
 * 设置流的各个参数
 * @param stream {ACL_VSTREAM*} 流指针
 * @param name {int} 所设置的参数类型中的第一个参数类型名,
 *  defined as ACL_VSTREAM_CTL_
 */
ACL_API void acl_vstream_ctl(ACL_VSTREAM *stream, int name,...);
 ...
 */
ACL_API acl_off_t acl_vstream_fseek(ACL_VSTREAM *stream, acl_off_t offset, int whence);
 ...
 * @deprecated 该函数的效率较低
 */
ACL_API acl_off_t acl_vstream_fseek2(ACL_VSTREAM *stream, acl_off_t offset, int whence);
 ...
 */
ACL_API int acl_vstream_getc(ACL_VSTREAM *stream);
#define	acl_vstream_get_char	acl_vstream_getc

/**
 * 从 stream 流中非阻塞地一次性最大读取 size 个字节
 * @param stream {ACL_VSTREAM*} 数据流指针
 ...
 *         n >= 0 正确.
 */
ACL_API int acl_vstream_nonb_readn(ACL_VSTREAM *stream, char *buf, int size);
 ...
 */
ACL_API int acl_vstream_probe_status(ACL_VSTREAM *stream);
 ...
 *  core 文件.
 */
ACL_API int acl_vstream_ungetc(ACL_VSTREAM *stream, int ch);
 ...
 *  失败而自动产生 core 文件!
 */
ACL_API int acl_vstream_unread(ACL_VSTREAM *stream, const void *ptr, size_t length);
 ...
 *  n > 0:  读到 了 n 个字节的数据.
 */
ACL_API int acl_vstream_gets_nonl(ACL_VSTREAM *stream, void *vptr, size_t maxlen);

/**
 * 从数据流中获得以字符串为标志结束位的内容
 * @param stream {ACL_VSTREAM*} 类型指针
 ...
 *  标志串, 则 stream 流中 (stream->flag & ACL_VSTREAM_FLAG_TAGYES) 不等于 0.
 */
ACL_API int acl_vstream_readtags(ACL_VSTREAM *stream, void *vptr, size_t maxlen,
 ...
 */
ACL_API int acl_vstream_readn(ACL_VSTREAM *stream, void *vptr, size_t maxlen);

/**
 * 将 stream 缓冲区内的数据拷贝到 vptr 中
 * @param stream {ACL_VSTREAM*} 数据流
 * @param vptr {void*} 用户的数据缓冲区指针地址
 * @param maxlen {size_t} vptr 数据缓冲区的空间大小
 * @return ret {int}, ret == ACL_VSTREAM_EOF: 表示出错, 应该关闭本地数据流,
 *  ret >= 0: 成功从 stream 数据流的缓冲区中读取了 ret 个字节的数据
 */
ACL_API int acl_vstream_bfcp_some(ACL_VSTREAM *stream, void *vptr, size_t maxlen);
...
*  ret >= 0: 成功从 stream 数据流的缓冲区中读取了 ret 个字节的数据
*/
ACL_API int acl_vstream_gets_peek(ACL_VSTREAM *stream, ACL_VSTRING *strbuf, int *ready);
 ...
 * @param ready {int*} 是否按要求读到所需数据的标志位指针, 不能为空
 * @return ret {int}, ret == ACL_VSTREAM_EOF: 表示出错, 应该关闭本地数据流,
 *  ret >= 0: 成功从 stream 数据流的缓冲区中读取了 ret 个字节的数据, 如果仅
 *  读到了一个空行, 则 ret == 0.
 */
ACL_API int acl_vstream_gets_nonl_peek(ACL_VSTREAM *stream, ACL_VSTRING *strbuf, int *ready);
/**
...
*/
ACL_API int acl_vstream_readn_peek(ACL_VSTREAM *stream, ACL_VSTRING *strbuf, int cnt, int *ready);
 ...
 *  ret >= 0: 成功从 stream 数据流的缓冲区中读取了 ret 个字节的数据.
 */
ACL_API int acl_vstream_read_peek(ACL_VSTREAM *stream, ACL_VSTRING *strbuf);
 ...
 */
ACL_API int acl_vstream_can_read(ACL_VSTREAM *stream);
 ...
 */
ACL_API int acl_vstream_fflush(ACL_VSTREAM *stream);
 ...
 */
ACL_API int acl_vstream_buffed_writen(ACL_VSTREAM *stream, const void *vptr, size_t dlen);
 ...
 */
ACL_API int acl_vstream_buffed_vfprintf(ACL_VSTREAM *stream, const char *fmt, va_list ap);
 ...
 */
#ifdef	WIN32
ACL_API int acl_vstream_buffed_fprintf(ACL_VSTREAM *stream, const char *fmt, ...);
#else
ACL_API int __attribute__((format(printf,2,3)))
	acl_vstream_buffed_fprintf(ACL_VSTREAM *stream, const char *fmt, ...);
#endif
 ...
 */
ACL_API int acl_vstream_buffed_fputs(const char *s, ACL_VSTREAM *stream);
...
*/
ACL_API int acl_vstream_write(ACL_VSTREAM *stream, const void *vptr, int dlen);
 ...
 */
ACL_API int acl_vstream_writev(ACL_VSTREAM *stream, const struct iovec *vector, int count);
 ...
 * @return {int} 返回成功写入的字节数，如果出错，则返回CL_VSTREAM_EOF
 */
ACL_API int acl_vstream_writevn(ACL_VSTREAM *stream, const struct iovec *vector, int count);
 ...
 *  ret > 0:  表示成功写了 dlen 个字节的数据
 */
ACL_API int acl_vstream_vfprintf(ACL_VSTREAM *stream, const char *fmt, va_list ap);
...
#else
ACL_API int __attribute__((format(printf,2,3)))
	acl_vstream_fprintf(ACL_VSTREAM *stream, const char *fmt, ...);
#endif
 ...
 * @return {int} 0 成功; ACL_VSTREAM_EOF 失败
 */
ACL_API int acl_vstream_fputs(const char *s, ACL_VSTREAM *stream);
 ...
 * @param stream {ACL_VSTREAM*} 数据流
 */
ACL_API void acl_vstream_free(ACL_VSTREAM *stream);
 ...
 * @param stream {ACL_VSTREAM*} 数据流
 */
ACL_API void acl_vstream_call_close_handles(ACL_VSTREAM *stream);
 ...
 */
ACL_API void acl_vstream_add_close_handle(ACL_VSTREAM *stream,
 ...
 * @param context {void*} close_fn 所需要的参数
 */
ACL_API void acl_vstream_delete_close_handle(ACL_VSTREAM *stream,
		void (*close_fn)(ACL_VSTREAM*, void*), void *context);
/**
 * 清除一个数据流中所有的关闭句柄
 * @param stream {ACL_VSTREAM*} 数据流
 */
ACL_API void acl_vstream_clean_close_handle(ACL_VSTREAM *stream);

/**
 * 重新复位数据流的内部数据指针及计数值
 * @param stream {ACL_VSTREAM*} 数据流
 */
ACL_API void acl_vstream_reset(ACL_VSTREAM *stream);
 ...
 */
ACL_API const char *acl_vstream_strerror(ACL_VSTREAM *stream);

/*-----------------------  以下为常用的宏函数 ------------------------------*/
/**
 * 从stream 流中读取一个字节的宏实现，效率要比 acl_vstream_getc()/1 高
 * @param stream {ACL_VSTREAM*} 数据流指针
 * @return {int} ACL_VSTREAM_EOF(出错) 或所读到的某个字节的ASCII,
