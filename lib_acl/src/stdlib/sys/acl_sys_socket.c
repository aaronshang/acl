...
int acl_socket_writev(ACL_SOCKET fd, const struct iovec *vector, int count,
	int timeout acl_unused, void *arg acl_unused)
{
	int   i, n, ret;
	...
	n = 0;
	for (i = 0; i < count; i++)	{
		ret = send(fd, vector[i].iov_base, vector[i].iov_len, 0);
		if (ret == SOCKET_ERROR) {
			errno = acl_last_error();
			return (-1);
		} else if (ret < (int) vector[i].iov_len) {
			n += ret;
			break;
		}
		n += ret;
