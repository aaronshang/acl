 ...
 * of bytes.
 */
void md5_update(md5_ctx *ctx, const void *_buf, unsigned len)
{
	...
	void* buf, size_t size)
{
	...
	md5_init(&context);
	if (k != NULL && len > 0)
		md5_update(&context, (const unsigned char *) k, (unsigned int) len);
	md5_update(&context, (const unsigned char*) s, (unsigned int) s_len);
	md5_final(digest, &context);
	memcpy(buf, digest, size);
	return ((const char*) buf);
}

const char* md5_string(const char *s, const char *k, size_t klen,
	char* buf, size_t size)
{
	...
	md5_init(&context);
	if (k != NULL && klen > 0)
		md5_update(&context, (const unsigned char *) k, (unsigned int) klen);
	md5_update(&context, (const unsigned char*) s, len);
	md5_final(digest, &context);
...
const char* md5_key2string(const unsigned char *s, size_t s_len,
	char* buf, size_t size)
{
	size_t i, n;
	char output1[34], *ptr;
